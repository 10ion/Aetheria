...
public class MoonfireCherryForestBiome extends AetheriaModElements.ModElement {

	@ObjectHolder("aetheria:moonfire_cherry_forest")
	public static final CustomBiome biome = null;

	public MoonfireCherryForestBiome(AetheriaModElements instance) {
		super(instance, 611);
	}
	...
	static class CustomBiome extends Biome {
		...
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.4f).depth(0.2f).scale(0.5f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.FOREST).waterColor(4159204).waterFogColor(329011).parent("aetheria:@registryname")
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(Blocks.GRASS_BLOCK.getDefaultState(),
							Blocks.DIRT.getDefaultState(), Blocks.DIRT.getDefaultState())));

			setRegistryName("moonfire_cherry_forest");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION,
					new CustomTreeFeature()
							.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(CherryLogItem.block.getDefaultState()),
									new SimpleBlockStateProvider(CherryLeavesItem.block.getDefaultState()))).baseHeight(6)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
				...
				if (!spawnTree) {
					...
					if (position.getY() < world.getHeight() - height - 1) {
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == CherryLeavesItem.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, CherryLeavesItem.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, CherryLogItem.block.getDefaultState(), bbox);
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == CherryLogItem.block.getDefaultState().getBlock()
					|| blockType == CherryLeavesItem.block.getDefaultState().getBlock()
