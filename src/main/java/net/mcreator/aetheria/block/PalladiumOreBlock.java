...
public class PalladiumOreBlock extends AetheriaModElements.ModElement {
	...
	public void init(FMLCommonSetupEvent event) {
		for (Biome biome : ForgeRegistries.BIOMES.getValues()) {
			...
			}, new OreFeatureConfig(OreFeatureConfig.FillerBlockType.create("palladium_ore", "palladium_ore", blockAt -> {
				boolean blockCriteria = false;
				if (blockAt.getBlock() == Blocks.STONE.getDefaultState().getBlock())
					blockCriteria = true;
				return blockCriteria;
			}), block.getDefaultState(), 6), Placement.COUNT_RANGE, new CountRangeConfig(1, 0, 0, 10)));
		}
	}
}
