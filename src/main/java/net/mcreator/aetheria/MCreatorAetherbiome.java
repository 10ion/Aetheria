...
public class MCreatorAetherbiome extends Elementsaetheria.ModElement {
	@ObjectHolder("aetheria:aetherbiome")
	public static final CustomBiome biome = null;

	public MCreatorAetherbiome(Elementsaetheria instance) {
		super(instance, 135);
	}
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder()
					...
					.waterColor(-13434829)
					.waterFogColor(-13434829)
					.surfaceBuilder(
							SurfaceBuilder.DEFAULT,
							new SurfaceBuilderConfig(MCreatorBluegrass.block.getDefaultState(), MCreatorMysticalStone.block.getDefaultState(),
									MCreatorMysticalStone.block.getDefaultState())));
	...
	static class CustomTreeFeature extends AbstractTreeFeature<NoFeatureConfig> {
		...
		public boolean place(Set<BlockPos> changedBlocks, IWorldGenerationReader worldgen, Random rand, BlockPos position, MutableBoundingBox bbox) {
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == MCreatorBluegrass.block.getDefaultState().getBlock() || ground == MCreatorMysticalStone.block.getDefaultState()
							.getBlock()) && (ground2 == MCreatorBluegrass.block.getDefaultState().getBlock() || ground2 == MCreatorMysticalStone.block
							.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MCreatorMysticalStone.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == MCreatorBlueleaf.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, MCreatorBlueleaf.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MCreatorMysticallog.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world,
												...
												MCreatorBlueleaf.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MCreatorMysticallog.block.getDefaultState().getBlock()
					|| blockType == MCreatorBlueleaf.block.getDefaultState().getBlock()
					|| blockType == MCreatorBluegrass.block.getDefaultState().getBlock()
					|| blockType == MCreatorMysticalStone.block.getDefaultState().getBlock();
		}
