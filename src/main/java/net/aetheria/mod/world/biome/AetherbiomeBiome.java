...
import net.aetheria.mod.block.MysticallogBlock;
import net.aetheria.mod.block.MysticalStoneBlock;
import net.aetheria.mod.block.BlueleafBlock;
import net.aetheria.mod.block.BluegrassBlock;
...
public class AetherbiomeBiome extends AetheriaModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.1f).depth(0.1f).scale(0.2f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(-13434829).waterFogColor(-13434829)
					.surfaceBuilder(SurfaceBuilder.DEFAULT, new SurfaceBuilderConfig(BluegrassBlock.block.getDefaultState(),
							MysticalStoneBlock.block.getDefaultState(), MysticalStoneBlock.block.getDefaultState())));
			setRegistryName("aetherbiome");
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(MysticallogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(BlueleafBlock.block.getDefaultState()))).baseHeight(7)
	...
	static class CustomTreeFeature extends AbstractTreeFeature<BaseTreeFeatureConfig> {
		...
		protected boolean place(IWorldGenerationReader worldgen, Random rand, BlockPos position, Set<BlockPos> changedBlocks,
			...
			if (position.getY() >= 1 && position.getY() + height + 1 <= world.getHeight()) {
				...
				if (!spawnTree) {
					...
					if (!((ground == BluegrassBlock.block.getDefaultState().getBlock()
							|| ground == MysticalStoneBlock.block.getDefaultState().getBlock())
							&& (ground2 == BluegrassBlock.block.getDefaultState().getBlock()
									|| ground2 == MysticalStoneBlock.block.getDefaultState().getBlock())))
						return false;
					BlockState state = world.getBlockState(position.down());
					if (position.getY() < world.getHeight() - height - 1) {
						setTreeBlockState(changedBlocks, world, position.down(), MysticalStoneBlock.block.getDefaultState(), bbox);
								...
								for (int i2 = position.getZ() - j1; i2 <= position.getZ() + j1; ++i2) {
									...
									if (Math.abs(position.getX()) != j1 || Math.abs(j2) != j1 || rand.nextInt(2) != 0 && i4 != 0) {
										...
										if (state.getBlock().isAir(state, world, blockpos) || state.getMaterial().blocksMovement()
												...
												|| state.getBlock() == BlueleafBlock.block.getDefaultState().getBlock()) {
											setTreeBlockState(changedBlocks, world, blockpos, BlueleafBlock.block.getDefaultState(), bbox);
										}
									}
								}
							}
						...
						for (int genh = 0; genh < height; genh++) {
							...
							setTreeBlockState(changedBlocks, world, genhPos, MysticallogBlock.block.getDefaultState(), bbox);
								...
								for (Direction Direction : Direction.Plane.HORIZONTAL) {
									if (rand.nextInt(4 - hlevel) == 0) {
										...
										setTreeBlockState(changedBlocks, world, position.add(dir.getXOffset(), height - 5 + hlevel, dir.getZOffset()),
												BlueleafBlock.block.getDefaultState(), bbox);
									}
								}
							}
						}
		...
		private boolean canGrowInto(Block blockType) {
			return blockType.getDefaultState().getMaterial() == Material.AIR || blockType == MysticallogBlock.block.getDefaultState().getBlock()
					|| blockType == BlueleafBlock.block.getDefaultState().getBlock() || blockType == BluegrassBlock.block.getDefaultState().getBlock()
					|| blockType == MysticalStoneBlock.block.getDefaultState().getBlock();
		}
