...
package net.mod.aetheria.world.dimension;

import org.jline.terminal.Size;

import net.mod.aetheria.item.TheAetherItem;
import net.mod.aetheria.block.BluegrassBlock;
import net.mod.aetheria.block.AetherFrameBlock;
...
public class TheAetherDimension extends AetheriaModElements.ModElement {
	...
	public void registerDimension(RegistryEvent.Register<ModDimension> event) {
		event.getRegistry().register(new CustomModDimension().setRegistryName("theaether"));
	}

	@SubscribeEvent
	public void onRegisterDimensionsEvent(RegisterDimensionsEvent event) {
		if (DimensionType.byName(new ResourceLocation("aetheria:theaether")) == null) {
			DimensionManager.registerDimension(new ResourceLocation("aetheria:theaether"), dimension, null, false);
		}
		type = DimensionType.byName(new ResourceLocation("aetheria:theaether"));
	}

	@Override
	public void init(FMLCommonSetupEvent event) {
		dimensionBiomes = new Biome[]{ForgeRegistries.BIOMES.getValue(new ResourceLocation("aetheria:aetherbiome")),};
	}

	@Override
	public void initElements() {
		elements.blocks.add(() -> new CustomPortalBlock());
		elements.items.add(() -> new TheAetherItem().setRegistryName("theaether"));
	}
	...
	public static class CustomPortalBlock extends NetherPortalBlock {
		public CustomPortalBlock() {
			...
			setRegistryName("theaether_portal");
		}
			...
			protected int getDistanceUntilEdge(BlockPos pos, Direction directionIn) {
				...
				for (i = 0; i < 22; ++i) {
					...
					if (!this.func_196900_a(this.world.getBlockState(blockpos))
							|| !(this.world.getBlockState(blockpos.down()).getBlock() == AetherFrameBlock.block.getDefaultState().getBlock())) {
						break;
					}
				}
				BlockPos framePos = pos.offset(directionIn, i);
				return (this.world.getBlockState(framePos).getBlock() == AetherFrameBlock.block.getDefaultState().getBlock()) ? i : 0;
			}
				...
				label56 : for (this.height = 0; this.height < 21; ++this.height) {
					for (int i = 0; i < this.width; ++i) {
						...
						if (i == 0) {
							...
							if (!(this.world.getBlockState(framePos).getBlock() == AetherFrameBlock.block.getDefaultState().getBlock())) {
								break label56;
							}
					...
					for (int i8 = 1; i8 < 3; ++i8) {
						for (int i9 = -1; i9 < 3; ++i9) {
							...
							this.world.setBlockState(blockpos$mutable,
									flag ? AetherFrameBlock.block.getDefaultState().getBlock().getDefaultState() : Blocks.AIR.getDefaultState());
						}
					}
				}
			}
			for (int k7 = -1; k7 < 3; ++k7) {
				for (int j8 = -1; j8 < 4; ++j8) {
					if (k7 == -1 || k7 == 2 || j8 == -1 || j8 == 3) {
						...
						this.world.setBlockState(blockpos$mutable, AetherFrameBlock.block.getDefaultState().getBlock().getDefaultState(), 3);
					}
				}
			}
		...
		public ChunkProviderModded(IWorld world, BiomeProvider provider) {
			super(world, provider, new EndGenerationSettings() {
				public BlockState getDefaultBlock() {
					return BluegrassBlock.block.getDefaultState();
				}
