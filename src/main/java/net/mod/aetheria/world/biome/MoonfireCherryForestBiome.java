...
import net.mod.aetheria.block.CherryLogBlock;
import net.mod.aetheria.block.CherryLeavesBlock;
...
public class MoonfireCherryForestBiome extends AetheriaModElements.ModElement {
	...
	static class CustomBiome extends Biome {
		public CustomBiome() {
			super(new Biome.Builder().downfall(0.3f).depth(0.1f).scale(0.3f).temperature(0.5f).precipitation(Biome.RainType.RAIN)
					.category(Biome.Category.NONE).waterColor(4159204).waterFogColor(329011).parent("aetheria:moonfire_cherry_forest")
			...
			addFeature(GenerationStage.Decoration.VEGETAL_DECORATION, new CustomTreeFeature()
					.withConfiguration((new BaseTreeFeatureConfig.Builder(new SimpleBlockStateProvider(CherryLogBlock.block.getDefaultState()),
							new SimpleBlockStateProvider(CherryLeavesBlock.block.getDefaultState()))).baseHeight(6)
			...
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(BlockbirdEntity.entity, 20, 4, 6));
			this.addSpawn(EntityClassification.CREATURE, new Biome.SpawnListEntry(EntityType.FOX, 1, 1, 1));
